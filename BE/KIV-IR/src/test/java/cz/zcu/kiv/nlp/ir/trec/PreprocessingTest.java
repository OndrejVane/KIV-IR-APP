...
import cz.zcu.kiv.nlp.ir.trec.core.preprocessing.*;
...
public class PreprocessingTest {
    ...
    private static void createNewInstance() {
        ...
        preprocessing.setTestingMode(true);
    }
    ...
    @Test
    public void testMyData_numbersWithComma() throws Exception {
        createNewInstance();

        preprocessing.index("10,");
        preprocessing.index("10, ");
        preprocessing.index("10");

        final Map<String, Integer> wordFrequencies = preprocessing.getWordFrequencies();
        printWordFrequencies(wordFrequencies);
        assertEquals(3, wordFrequencies.get(preprocessing.getProcessedForm("10")).intValue());

    }

    @Test
    public void testMyData_dateFormatWithSpaces() throws Exception {
        createNewInstance();

        preprocessing.index("10. 3. ");
        preprocessing.index("10. 3.");
        preprocessing.index("10.3.");
        preprocessing.index("10. 3. 2011");
        preprocessing.index("10.3.2011");

        final Map<String, Integer> wordFrequencies = preprocessing.getWordFrequencies();
        printWordFrequencies(wordFrequencies);
        assertEquals(3, wordFrequencies.get(preprocessing.getProcessedForm("10.3.")).intValue());
        assertEquals(2, wordFrequencies.get(preprocessing.getProcessedForm("10.3.2011")).intValue());

    }

    /**
     * Test load some data from json file, test if something has been load
     * and print all processed words.
     *
     * @throws Exception
     */
    @Test
    public void testMyData_loadAndProcessMyData() throws Exception {
        createNewInstance();
        ...
        // check if is 10 articles in lis
        assertEquals(1000, articles.size());

        for (Article article : articles) {
            preprocessing.index(article.getContent());
        }

        printWordFrequencies(preprocessing.getWordFrequencies());
    }
}
